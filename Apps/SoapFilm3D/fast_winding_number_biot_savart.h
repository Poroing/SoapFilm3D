...
template<typename DerivedP,
         ...
         typename DerivedEC>
IGL_INLINE void
fast_winding_number_biot_savart(const Eigen::MatrixBase<DerivedP>& P,
...
template<typename DerivedP,
         ...
         typename DerivedEC>
IGL_INLINE void
igl::fast_winding_number_biot_savart(const Eigen::MatrixBase<DerivedP>& P,
                                     const Eigen::MatrixBase<DerivedG>& G,
                                     const std::vector<std::vector<Index>>& point_indices,
                                     const Eigen::MatrixBase<DerivedCH>& CH,
                                     int expansion_order,
                                     ...
                                     Eigen::PlainObjectBase<DerivedEC>& EC)
{
    ...
    helper = [&helper, &P, &G, &expansion_order, &point_indices, &CH, &EC, &R, &CM](
        ...
        for (size_t curr_point_index : point_indices[index])
        {
            ...
            if (EC.cols() >= (3 + 9))
            {
                TempCoeffs = point.transpose() * G.row(curr_point_index);
                EC.block(index, 3, 1, 9) +=
                  Eigen::Map<Eigen::Matrix<real_ec, 1, 9>>(TempCoeffs.data(), TempCoeffs.size());
            }
            ...
            if (EC.cols() == (3 + 9 + 27))
            {
                for (int k = 0; k < 3; k++)
                {
                    ...
                    EC.block(index, 12 + 9 * k, 1, 9) += Eigen::Map<Eigen::Matrix<real_ec, 1, 9>>(
                      TempCoeffs.data(), TempCoeffs.size());
                }
            }
        }
                                     ...
                                     Eigen::PlainObjectBase<DerivedWN>& WN)
{
    ...
    typedef Eigen::Matrix<typename DerivedEC::Scalar, 1, DerivedEC::ColsAtCompileTime> ECRow;

    typedef Eigen::Matrix<real_q, 1, 3> RowVec;
    typedef Eigen::Matrix<real_wn, 1, 3> WnRowVec;
    typedef Eigen::Matrix<real_ec, 3, 3> EC_3by3;

    auto direct_eval = [&PI_4, delta](const RowVec& loc,
        ...
        if (EC.row(child_index).size() > 3 + 9)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                for (int u = 0; u < 3; u++)
                {
                    ...
                    {
                        if (u == v)
                            RowCol_Diagonal(u, v) += loc(i);
                        if (u == i)
                            RowCol_Diagonal(u, v) += loc(v);
                        if (v == i)
                            RowCol_Diagonal(u, v) += loc(u);
                    }
                }
    ...
    helper =
      [&helper, &P, &G, &point_indices, &CH, &CM, &R, &EC, &beta, &direct_eval, &expansion_eval](
        ...
        for (int i = 0; i < near_indices.size(); i++)
        {
            ...
            if (CH(index, 0) == -1)
            {
                for (int j = 0; j < point_indices[index].size(); j++)
                {
                    ...
                    wn += direct_eval(P.row(curr_row) - query, G.row(curr_row));
                }
            }
            // Non-Leaf Case
            else
