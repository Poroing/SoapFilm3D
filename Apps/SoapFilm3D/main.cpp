...
void
renderBitmapString(float x, float y, float z, std::string s)
{
    glColor3f(0, 0, 0);
    glRasterPos3f(x, y, z);
...
display()
{
    ...
    g_sim.render(
      g_sc.render_mode,
      Vec2d((double)g_sc.mouse_x / g_sc.win_w * 2 - 1, 1 - (double)g_sc.mouse_y / g_sc.win_h * 2),
      g_sc.selection_mode);
...
keyboard(unsigned char k, int x, int y)
{
    ...
    {
        g_sc.render_mode =
          (Sim::RenderMode)(((int)g_sc.render_mode + (k == 'm' ? 1 : -1)) % ((int)Sim::RM_COUNT));
    ...
    {
        g_sc.selection_mode =
          (k == 'e' ? (g_sc.selection_mode | Sim::SM_EDGE) : (g_sc.selection_mode & ~Sim::SM_EDGE));
        displayCurrentSelectingMode();
        glutPostRedisplay();
    }
    else if (k == 'f' || k == 'F')
    {
        g_sc.selection_mode =
          (k == 'f' ? (g_sc.selection_mode | Sim::SM_FACE) : (g_sc.selection_mode & ~Sim::SM_FACE));
        displayCurrentSelectingMode();
        glutPostRedisplay();
    }
    else if (k == 'n' || k == 'N')
