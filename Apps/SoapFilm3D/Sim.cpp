...
Sim::Sim(bool verbose)
  : m_verbose(verbose), m_scene("unspecified"), m_output_directory(""), m_vs(NULL), m_dt(0),
    ...
    m_nearest_face(-1), m_prrenderer(NULL)
{
}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool
Sim::init(const std::string& option_file, bool save_outputs, bool headless)
{
    ...
    Options::addDoubleOption(
      "lostopos-min-triangle-area-fraction",
      0.02); // minimum allowed triangle area (fraction of mean edge length squared)
    Options::addBooleanOption("lostopos-t1-transition-enabled", true); // whether t1 is enabled
    Options::addDoubleOption("lostopos-t1-pull-apart-distance-fraction",
                             0.1); // t1 pull apart distance (fraction of mean edge legnth)
    Options::addBooleanOption("lostopos-smooth-subdivision",
                              false); // whether to use smooth subdivision during remeshing
    Options::addBooleanOption("lostopos-allow-non-manifold",
    ...
    if (m_scene == "load")
    {
        ...
        std::unique_ptr<VS3D> vs = std::make_unique<VS3D>(std::vector<LosTopos::Vec3d>(),
    ...
    else
    {
        ...
        if (m_scene == "sphere")
            ...
            m_vs = Scenes::sceneDoubleBubble(
              this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "twobubbles")
            m_vs = Scenes::sceneTwoBubbles(
              this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "triplejunction")
            m_vs = Scenes::sceneTripleJunction(
              this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "foaminit")
            m_vs = Scenes::sceneFoamInit(
              this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "foam")
            m_vs = Scenes::sceneFoam(
              this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "quadjunction")
            m_vs = Scenes::sceneQuadJunction(
              this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "constrainedsphere")
            m_vs = Scenes::sceneConstrainedSphere(
              this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "bubblewand")
            m_vs = Scenes::sceneBubbleWand(
              this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "tworingspinching")
            m_vs = Scenes::sceneTwoRingsPinching(
              this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "pullingfoam")
            m_vs = Scenes::scenePullingFoam(
              this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "peanutbubble")
            m_vs = Scenes::scenePeanutBubble(
              this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "straw")
            m_vs = Scenes::sceneStraw(
              this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
        else if (m_scene == "carousel")
            m_vs = Scenes::sceneCarousel(
              this, vertices, faces, face_labels, constrained_vertices, constrained_positions);
...
Sim::stepOutput(bool headless)
{
    if (m_output_directory != "")
    {
        ...
        if ((pngfd == 0 || frameid % pngfd == 0) && Options::boolValue("output-png") && !headless)
        {
            ...
            png_ss << m_output_directory << "/frame" << std::setfill('0') << std::setw(6)
                   << (frameid / std::max(pngfd, 1)) << ".png";
...
{
bool
is_edge_nonmanifold(const LosTopos::SurfTrack& st, size_t e)
{
    return st.m_mesh.m_edge_to_triangle_map[e].size() != 2;
}

bool
is_vertex_nonmanifold(const LosTopos::SurfTrack& st, size_t v)
{
...
bool
is_face_next_to_nonmanifold_vertices(const LosTopos::SurfTrack& st, size_t f)
{
...
bool
is_edge_next_to_nonmanifold_vertices(const LosTopos::SurfTrack& st, size_t e)
{
...
bool
is_vertex_next_to_nonmanifold_vertices(const LosTopos::SurfTrack& st, size_t v)
{
    for (size_t i = 0; i < st.m_mesh.m_vertex_to_edge_map[v].size(); i++)
...
void
Sim::render(RenderMode rm, const Vec2d& mousepos, int selection_mask)
{
    if (rm == RM_PR)
    {
        m_prrenderer->render();
    ...
    Mat4d PJ;
    {
        ...
        MV << mv[0], mv[4], mv[8], mv[12], mv[1], mv[5], mv[9], mv[13], mv[2], mv[6], mv[10],
          mv[14], mv[3], mv[7], mv[11], mv[15];
        PJ << pj[0], pj[4], pj[8], pj[12], pj[1], pj[5], pj[9], pj[13], pj[2], pj[6], pj[10],
          pj[14], pj[3], pj[7], pj[11], pj[15];
    }
    Mat4d MVP = PJ * MV;
    ...
    if (selection_mask & SM_EDGE)
    {
        for (size_t i = 0; i < m_vs->mesh().ne(); i++)
        {
            ...
            //            double distance = (mousepos - scrv0 - (mousepos - scrv0).dot(scrv1 -
            //            scrv0) * (scrv1 - scrv0) / (scrv1 - scrv0).squaredNorm()).norm();
            if (distance < mind || mind < 0)
            {
                mind = distance;
                m_nearest_vertex = -1;
    ...
    if (rm != RM_OPAQUE_SMOOTH_SHADED)
    {
        ...
        for (size_t i = 0; i < m_vs->mesh().ne(); i++)
        {
                ...
                && m_vs->isVertexConstrained(m_vs->mesh().m_edges[i][1]))
            {
                if (rm == RM_NONMANIFOLD)
                {
                    //                if (!is_edge_next_to_nonmanifold_vertices(*m_vs->surfTrack(),
                    //                i))
                    if (!is_edge_nonmanifold(*m_vs->surfTrack(), i))
                        continue;
                }
    ...
    if (rm != RM_OPAQUE_SMOOTH_SHADED)
    {
        ...
        for (size_t i = 0; i < m_vs->surfTrack()->m_mesh.nv(); i++)
        {
            if (rm == RM_NONMANIFOLD)
            {
                //                if (!is_vertex_next_to_nonmanifold_vertices(*m_vs->surfTrack(),
                //                i))
    ...
    if (rm != RM_OPAQUE_SMOOTH_SHADED)
    {
        ...
        for (size_t i = 0; i < m_vs->mesh().nt(); i++)
        {
            ...
            LosTopos::Vec3st t = m_vs->mesh().get_triangle(i);
            if (m_vs->m_st->vertex_is_any_solid(t[0]) && m_vs->m_st->vertex_is_any_solid(t[1])
            ...
            Vec3d gamma =
              -(e01 * (*m_vs->m_Gamma)[t[2]].get(l) + e12 * (*m_vs->m_Gamma)[t[0]].get(l)
...
Sim::showPrimitiveInfo()
{
    if (m_nearest_vertex >= 0)
    {
        ...
        for (size_t i = 0; i < m_vs->mesh().m_vertex_to_edge_map[m_nearest_vertex].size(); i++)
            std::cout << " " << m_vs->mesh().m_vertex_to_edge_map[m_nearest_vertex][i];
        std::cout << std::endl;
        std::cout << "  incident faces:";
        for (size_t i = 0; i < m_vs->mesh().m_vertex_to_triangle_map[m_nearest_vertex].size(); i++)
            std::cout << " " << m_vs->mesh().m_vertex_to_triangle_map[m_nearest_vertex][i];
        std::cout << std::endl;
        //        std::cout << "  ====== " << (*m_vs->m_Gamma)[m_nearest_vertex][0] -
        //        (*m_vs->m_Gamma)[m_nearest_vertex][1] << std::endl; if (m_vs->m_dbg_v1.size() ==
        //        m_vs->mesh().nv())
    ...
    if (m_nearest_edge >= 0)
    {
        std::cout << "Edge of Interest: " << m_nearest_edge << ": "
                  ...
                  << m_vs->pos(m_vs->mesh().m_edges[m_nearest_edge][1]).transpose() << ") length = "
        ...
        // m_vs->m_dbg_e1[m_nearest_edge][1] << " " << m_vs->m_dbg_e1[m_nearest_edge][2] <<
        // std::endl;
        std::cout << "  incident faces:";
        for (size_t i = 0; i < m_vs->mesh().m_edge_to_triangle_map[m_nearest_edge].size(); i++)
            std::cout << " " << m_vs->mesh().m_edge_to_triangle_map[m_nearest_edge][i];
        std::cout << std::endl;
    }
    ...
    if (m_nearest_face >= 0)
    {
        ...
        for (size_t i = 0; i < 3; i++)
            std::cout << " " << m_vs->mesh().m_triangle_to_edge_map[m_nearest_face][i];
        std::cout << std::endl;
    }
}
